...
def split_address(registration_no,config_dict,db_config):
    ...
    cursor = connection.cursor()
    connection.autocommit = True
    business_address_query = f"select ba_full_address from Company where registration_no = '{registration_no}'"
        ...
        try:
            ...
            if str(business_address_to_split).lower() != 'null' and business_address_to_split is not None:
                ...
                try:
                    ...
                    pass
                update_query = f"update Company set ba_city = '{city}',ba_state = '{state}',ba_pincode = '{pincode}',ba_address_line1 = '{address_line1}',ba_address_line2 = '{address_line2}' where registration_no = '{registration_no}' and ba_full_address = '{business_address_to_split}'"
                logging.info(update_query)
                cursor.execute(update_query)
                cursor.close()
                connection.close()
